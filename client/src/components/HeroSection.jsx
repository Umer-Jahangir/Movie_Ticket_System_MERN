...
import { StarIcon, Clock, Play, CalendarDays, PlayCircleIcon } from 'lucide-react';
...
const HeroSection = () => {
  ...
  const nextMovie = () => {
    setCurrentMovie((prev) => (prev + 1) % dummyShowsData.length);
  };
    ...
    <div
      ...
      style={{
        backgroundImage: `linear-gradient(to right, rgba(0,0,0,0.8), rgba(0,0,0,0.5)),
          url(${dummyShowsData[currentMovie].backdrop_path})`
      }}
    >
      
      <h1 className="text-4xl md:text-6xl font-bold text-white mb-2 mt-6 ">
        {dummyShowsData[currentMovie].title}
      </h1>
      ...
      <div className="flex items-center gap-4 text-gray-300 text-sm mb-4">
        ...
        <div className="flex items-center gap-2 mt-2 text-white/80 text-sm">
          ...
          <span>{dummyShowsData[currentMovie].release_date}</span>
        </div>

        <div className="flex items-center gap-1">
          <StarIcon className="w-4 h-4 text-yellow-400" />
          <span>{dummyShowsData[currentMovie].vote_average.toFixed(1)}</span>
        </div>
        <div className="flex items-center gap-1">
          <Clock className="w-4 h-4" />
          <span>{timeFormat(dummyShowsData[currentMovie].runtime)}</span>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {dummyShowsData[currentMovie].genres.map((genre) => (
      ...
      <p className="text-gray-300 text-lg md:text-xl max-w-2xl mb-8">
        {dummyShowsData[currentMovie].overview}
      </p>

      <div className="flex items-center gap-4">
        <button
          onClick={() =>
            navigate(`/movies/${dummyShowsData[currentMovie].id}#dateSelect`)
          }
        ...
        <button
            onClick={() =>
              navigate('/Trailer', {
                state: {
                  trailer: dummyShowsData[currentMovie].trailerUrl,
                  title: dummyShowsData[currentMovie].title
                }
              })
            }
